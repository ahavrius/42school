...
static void	display_stack(t_data *data, t_stack *a, float range, int who)
{
	...
	while (current != a)
	{
		put_rect(data, init_point(MARGIN + 250 - range * ABS(current->value) + 500 * who,
			1000 + MARGIN  - (i + 1) * STACK_H),
			init_point(MARGIN + 250 + range * ABS(current->value) + 500 * who,
			1000 + MARGIN  - i * STACK_H), (current->value > 0) ? COLOR_P : COLOR_N);
		current = current->prev;
		i++;
	}//for a
		put_rect(data, init_point(MARGIN + 250 - range * ABS(a->value) + 500 * who,
			1000 + MARGIN  - (i + 1) * STACK_H),
